...
_DEFAULT_TAIL_TOKEN_BUDGET = 8_000
...
class ContextCompressor:
    ...
    def _serialize_for_summary(self, turns: List[Dict[str, Any]]) -> str:
        ...
        for msg in turns:
            ...
            # Tool results: retain enough for file paths, errors, code details
            if role == "tool":
        ...
        # Pre-extract file paths so the summarizer sees them explicitly listed
...
## Relevant Files
[Files read, modified, or created — with FULL paths and brief note on each. Accumulate across compactions. Mark each as (read), (modified), or (created).]

## Critical Context
[Specific values, error messages, configuration details, port numbers, URLs, branch names, version strings — anything that would be lost without explicit preservation. Preserve verbatim.]
...
[Important technical decisions and why they were made. User preferences, coding style, constraints — accumulate across compactions.]

## Next Steps
[What needs to happen next to continue the work]

MANDATORY PRESERVATION RULES:
- Every file path mentioned in the turns MUST appear in the summary (e.g., /home/user/project/src/foo.py)
- Every error message or exception MUST be preserved verbatim, not paraphrased
- Every command that was executed MUST be listed with its exact invocation
- Every specific value (port numbers, version strings, config values, URLs, branch names) MUST be kept exactly as-is
- DO NOT use vague descriptions like "several files were modified" or "an error occurred" — always be exact

Target ~{summary_budget} tokens. The goal is to prevent the next assistant from repeating work or losing important details.
...
## Relevant Files
[Files read, modified, or created — with FULL paths and brief note on each. Mark each as (read), (modified), or (created).]
...
## Decisions & Constraints
[Important technical decisions and why they were made. User preferences, coding style, constraints.]
